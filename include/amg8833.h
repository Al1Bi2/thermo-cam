...
public:
    AMG8833(){};
    bool init(int SDA = I2C_SDA_PIN, int SCL = I2C_SCL_PIN){
    ...
    void reset(){
        ...
        I2C_STATUS result=write_register(RESET_REGISTER, 0x3F);
        Serial.println(static_cast<bool>(result));
    }
    ...
    std::array<std::array<float, 8>, 8> get_matrix(){
        ...
        for(int i = 0; i < 8; i++){
            ...
            for(int j = 0; j < 8; j++){
                ...
                temp_matrix[i][j] = convert_12bit(read_register16(TEMPERATURE_REGISTER_START + (i*2 * 8) + j*2))* 0.25;
            }
        }
        return temp_matrix;
    }
    float get_thermistor_temperature(){
        return convert_12bit(read_register16(THERMISTOR_REGISTER_LSB));
    }
private:
    ...
    I2C_STATUS write_register(uint8_t address, uint8_t value){
        i2c->beginTransmission(AMG8833_ADDRESS);
        i2c->write(address);
        i2c->write(value);
        if(i2c->endTransmission() != 0){
    ...
    float convert_12bit(uint16_t value){
